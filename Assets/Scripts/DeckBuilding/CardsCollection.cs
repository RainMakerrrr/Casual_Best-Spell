...
namespace DeckBuilding
{
    public class CardsCollection : MonoBehaviour
    {
        ...
        {
            var duplicates = _spellsPool.SpellsCollection.GroupBy(x => x)
                .SelectMany(g => g.Skip(1)).ToList();

            var distinctCollection = _spellsPool.SpellsCollection.Distinct().ToList();
        ...
        private void FindDuplicates(IReadOnlyCollection<SpellData> duplicates, IReadOnlyCollection<SpellData> spells)
        {
            ...
            _spellsPool.RemoveCollectionFromDeck(duplicates.Skip(1));
        }
        ...
        public Transform GetDisabledSpellTransform(SpellCard spellCard, out int index)
        {
